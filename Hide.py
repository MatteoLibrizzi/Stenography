...
class Messagge:
	...
	def splitmsg(self):
		...
		if(len(self.bit)%2==0):#checks if numbers of bits is odd
			nOfGroups=int(len(self.bit)/2)#if not everything is fine
			earlyStop=False
		else:
			nOfGroups=int(math.ceil(len(self.bit)/2))#if it is, it is rounded up, as to not go out of index later
			earlyStop=True
		k=0
		while a<nOfGroups*2:#loops through the list to fill without exceeding it
			slots.append([])#appends a new list (gonna be filled with 2 elements)
			for i in range(2):

				if(not(earlyStop and (a==(nOfGroups*2)-1))):#if it is odd AND it is in the last iterarion it does nothing, otherwise appends a new bit
